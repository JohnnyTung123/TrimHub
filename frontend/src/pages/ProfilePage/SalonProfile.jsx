  ...
  const fetchSalonInfo = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/salon-info?name=${user.username}`
      );
      if (!response.ok) {
    ...
    try {
      console.log(name);
      const response = await fetch(
        `http://localhost:8080/salon-info/image?name=${user.username}`
      );
      if (!response.ok) {
        throw Error("Cannot fetch data.");
      }
              ...
              <div className="flex flex-col pr-12 pl-4 mt-8 max-md:pr-5">
                <div className="flex flex-col justify-center self-end max-w-full text-3xl text-white rounded-xl bg-green-700 bg-opacity-80 shadow-[0px_4px_4px_rgba(0,0,0,0.25)] w-[282px]">
                  <button
                    ...
                    className="justify-center px-6 py-4 rounded-xl border border-white border-solid bg-green-700 bg-opacity-80 shadow-[0px_4px_4px_rgba(0,0,0,0.25)] max-md:px-5"
                  >
                    Upload new photo
                  </button>
                </div>
